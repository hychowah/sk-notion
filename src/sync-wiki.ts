...
import { getPageSummary, replacePageContent, updatePageDetails } from "./notion/page";
...
function resolveNotionUrl(
  ...
  for (const [_key, entry] of Object.entries(pageMap.sections)) {
    if (entry.file === localFilePath) {
      const pageId = entry.pageId === "root"
        ? rootPageId
...
function resolveInternalLinks(
  ...
  return markdown.replace(
    ...
    (_match, text: string, linkPath: string) => {
      ...
      // Not a known wiki page — leave as-is
      return _match;
    },
  );
}
...
export async function syncWikiSection(
  ...
  const { client, rootPageId, wikiContentDirectory, pageMap } = deps;
  ...
  if (dryRun) {
    ...
    return {
      ...
      url: `https://app.notion.com/p/${encodeURIComponent(section.name.replace(/\s+/g, "-"))}-${pageId.replace(/-/g, "")}`,
      uploadedImages: [],
      warnings: [],
    };
  }
  ...
  const transformed = markdownToContentNodes(
    resolveInternalLinks(markdownInput, filePath, wikiContentDirectory, pageMap, rootPageId),
  );
